...
CREATE INDEX idx_articles_is_starred ON articles(is_starred) WHERE is_starred = TRUE;
...
-- Feeds: Users can only access own feeds
...
-- Articles: Users can only read articles from own feeds
CREATE POLICY "Users can read own articles" ON articles FOR SELECT 
  USING (feed_id IN (SELECT id FROM feeds WHERE user_id = auth.uid()));
CREATE POLICY "Users can update own articles" ON articles FOR UPDATE 
  USING (feed_id IN (SELECT id FROM feeds WHERE user_id = auth.uid()));
CREATE POLICY "Users can insert own articles" ON articles FOR INSERT 
